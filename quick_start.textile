...
[-If you use @sbt@, add the following dependency to your project definition class and then run "@sbt update@".  Maven configuration and-] manual download links are "here":download.html.

{% highlight scala %}
  val scala_stm = "org.scala-lib" %% "scala-stm" % "0.1-SNAPSHOT"
{% endhighlight %}
...
Another way to fix an atomic block that ends in @retry@ is to provide an alternative.  You can chain atomic blocks using @orAtomic@; the lower alternatives will be tried if the upper ones call @retry@.  This lets you compose functions that block using @retry@, or to convert to and from blocking behavior.
...
ScalaSTM might need to try an atomic block more than once before optimistic concurrency can succeed.  This means that any call to the STM might detect a speculation failure, roll back the transaction, and then restart it.  Local non-@Ref@ variables that have a lifetime longer than the atomic block won't be rolled back, and so they should be avoided.  Local variables inside the atomic block are fine, though.

@badToString@ is incorrect because it uses a mutable @StringBuilder@ both outside and inside its atomic block.  The return value will definitely mention all of the elements of the list, but it might mention some of them two or more times.  @toString@ is correct because it uses a new @StringBuilder@ for each atomic attempt.
...
This list example is part of the source on GitHub: "ConcurrentIntList.scala":https://github.com/nbronson/scala-stm-api/blob/master/src/test/scala/scala/concurrent/stm/examples/ConcurrentIntList.scala

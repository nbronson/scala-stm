...
** for all 2.11 releases: ["scala-stm_2.11-0.7.jar":https://oss.sonatype.org/content/repositories/releases/org/scala-stm/scala-stm_2.11/0.7/scala-stm_2.11-0.7.jar]
...
Separate artifacts are published for each version of Scala, so the desired Scala version (such as 2.11) must be included in the @artifactId@ for Maven (@sbt@ automatically handles this). For Scala versions before 2.10 this includes the minor version as well (such as 2.9.3). The ScalaSTM dependency for your @pom.xml@ is

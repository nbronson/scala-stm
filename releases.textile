...
ScalaSTM builds for 2.9.0 and 2.9.0-1 have identical bytecode, but for the benefit of sbt users both are provided.
...
Separate artifacts are published for each version of Scala, so the desired Scala version (such as 2.9.0) must be included in the @artifactId@ for Maven (@sbt@ automatically handles this). The ScalaSTM dependency for your @pom.xml@ is

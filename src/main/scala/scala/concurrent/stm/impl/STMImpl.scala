...
object STMImpl {
   ...
   *  `implClassName` was previously selected.
   *
   *  @throws IllegalStateException if a different STM implementation has
   ...
   *  selected instance.
   *
   *  @throws IllegalStateException if an STM implementation has already been
   *      selected and `impl != instance`
   */

...
private[stm] class StubSTMImpl extends impl.STMImpl {
  ...
  def timeout: Option[Long] = throw new AbstractMethodError
  def withTimeout(timeoutMillis: Long): TxnExecutor = throw new AbstractMethodError
  def withNoTimeout: TxnExecutor = throw new AbstractMethodError

...
object Test {
  ...
  def main(args: Array[String]) {
    ...
    def value(k: Int) = "x" + k
    def kvRange(b: Int, e: Int) = (b until e) map { i => (i -> value(i)) }
    ...
    test("contention") {
      ...
      for (pass <- 0 until 2) {
        ...
        if (false) println("TMap: contended: " + numThreads + " threads, total throughput was " + (elapsed / numThreads) + " nanos/op")
      }
    }
    ...
    if ("slow" == "enabled") test("sequential non-txn read performance") {
      for (pass <- 0 until 4) {
        for (size <- List(10, 100, 1000, 100000)) {
          ...
          val elapsed = now - t0
          if (false) println("TMap: non-txn read: " + size + " keys/map -> " + elapsed + " nanos/contain")
        }
      }
    }

    if ("slow" == "enabled") test("sequential non-txn append performance") {
      for (pass <- 0 until 2) {
        for (size <- List(10, 100, 1000, 100000)) {
          ...
          val elapsed = now - t0
          if (false) println("TMap: non-txn append: " + size + " keys/map -> " + elapsed + " nanos/added-key")
        }
      }
    }

    if ("slow" == "enabled") test("sequential non-txn update performance") {
      ...
      for (pass <- 0 until 2) {
        for (size <- List(10, 100, 1000, 100000)) {
          ...
          val elapsed = now - t0
          if (false) println("TMap: non-txn update: " + size + " keys/map -> " + elapsed + " nanos/put")
        }
      }
    }

    if ("slow" == "enabled") test("sequential non-txn put/remove mix performance") {
      val values = (0 until 37) map { "x" + _ }
      ...
      for (pass <- 0 until 4) {
        for (size <- List(10, 100, 1000, 100000)) {
          ...
          val elapsed = now - t0
          if (false) println("TMap: non-txn put/remove: " + size + " keys/map -> " + elapsed + " nanos/op")
        }
      }
    }

    if ("slow" == "enabled") test("sequential txn read performance") {
      for (txnSize <- List(2, 10, 1000)) {
        for (pass <- 0 until 2) {
          for (size <- List(10, 100, 1000, 100000)) {
            ...
            val elapsed = now - t0
            if (false) println("TMap: txn read: " + txnSize + " accesses/txn: " + size + " keys/map -> " + elapsed + " nanos/op")
          }
        }
      }
    }

    if ("slow" == "enabled") test("sequential txn put/remove mix performance") {
      val values = (0 until 37) map { "x" + _ }
      val rand = new skel.SimpleRandom
      for (txnSize <- List(2, 10, 1000)) {
        for (pass <- 0 until 2) {
          for (size <- List(10, 100, 1000, 100000)) {
            ...
            val elapsed = now - t0
            if (false) println("TMap: txn put/remove: " + txnSize + " accesses/txn: " + size + " keys/map -> " + elapsed + " nanos/op")
          }
        }
      }
    }

...
object Test {
  ...
  def main(args: Array[String]) {
    ...
    if ("slow" == "enabled") test("default flipper test") {
      Config(
        DEFAULT_SYNC_COUNT,
        DEFAULT_TRANS_COUNT,
        DEFAULT_INSTR_COUNT,
    ...
    if ("slow" == "enabled") test("default flipper test w/TArray") {
      Config(
        DEFAULT_SYNC_COUNT,
        DEFAULT_TRANS_COUNT,
        DEFAULT_INSTR_COUNT,
    ...
    if ("slow" == "enabled") test("random flipper test") {
      for (i <- 0 until 1) {
        Config(
          DEFAULT_SYNC_COUNT,
          DEFAULT_TRANS_COUNT,
    ...
    case class Config(syncCount: Int,
      ...
      val len = syncCount*transCount*instrCount*threadCount
      val rand = new java.util.Random(randSeed)
    ...
    def parallelRun(tasks: Iterable[() => Unit]) {

...
object Test {
  ...
  def main(args: Array[String]) {
    ...
    def runTest(numRefs: Int, numReads: Int, numWrites: Int, numThreads: Int, txnSize: Int, nested: Boolean, name: String) {
      ...
      if (false) printf("ContentionSuite: %s, %d nanos/op aggregate throughput\n", name, nanosPerOp)
    }

    test("starving elder small") { runElderTest(8, 10) }
    if ("slow" == "enabled") test("starving elder large") { runElderTest(32, 100) }
    ...
    def runElderTest(writerCount: Int, numElders: Int) {
      ...
      while (eldersLeft.single() > 0) {
        ...
        assert(sum == n * (n - 1) / 2)
        eldersLeft.single -= 1
        if (false) println("elder ran " + tries + " times")
      }
    ...
    test("colliding trySet") {
      ...
      if (false) println(total.single() + " rejected trySet-s")
    }
  }
}

...
object Test {
  ...
  def main(args: Array[String]) {
    ...
    test("perf 2") {
      ...
      if (false) println("commit barrier, 2 threads, " + (elapsed / count) + " nanos/barrier")
    }

    test("perf 10") {
      val count = 2000
      val elapsed = runStress(10, count, false)
      if (false) println("commit barrier, 10 threads, " + (elapsed / count) + " nanos/barrier")
    }
        ...
        val z = cb.addMember().atomic { implicit txn =>
          ...
          val t = new Thread() {
            override def run() {
              m.atomic { implicit txn =>
                y() = i
                Txn.afterCommit({ _ => commits.incrementAndGet() })(txn)
              }
            }
          }
          t.start()
          Txn.afterCommit({ _ => t.join() })(txn)
          if (i < 10)
